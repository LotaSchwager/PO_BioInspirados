...
from Metaheuristics.imports import iterarGWO,iterarPSA,iterarSCA,iterarWOA,IterarPO
...
def solverB(id, mh, maxIter, pop, function, lb, ub, dim):
    ...
    def fo(x):
        x = np.array(x)
        for j in range(len(x)): x[j] = np.clip(x[j], lb[j], ub[j]) # Reparación de soluciones
        return x,f(function,x) # Return de la solución reparada y valor de función objetivo
    
    # En caso de que sea la metaheuristica PSO se inicializara la clase PO
    if mh == 'PO':
